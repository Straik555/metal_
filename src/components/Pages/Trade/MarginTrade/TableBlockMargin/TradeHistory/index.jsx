...
import { tradeHistoryMarginSelector } from '../../../../../../redux/trade/margin/selectors';
...
const TradeHistory = () => {
  ...
  const history = useSelector(tradeHistoryMarginSelector);
  ...
  const handleSearch = () => {
    dispatch({
      type: types.MARGIN_GET_TRADE_HISTORY_START,
      payload: {
        params: {
          per_page: 20,
            ...
            {!history?.length ? (
              ...
              history.map(data => (
                <tr key={data?.id} className="table_row change">
                  ...
                  <td>{toCrop(8)(data?.price_filled)}</td>
                  <td>{toCrop(8)(data?.quantity_remaining)}</td>
                  <td>{toCrop(8)(data?.fee)}</td>
